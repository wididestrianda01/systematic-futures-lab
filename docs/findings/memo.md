...
2. **A null model is a real competitor.** The vol-targeted buy-and-hold book earns Sharpe 0.41 at
   2 bps in develop+validate on book turnover of 0.007, which beats the trend benchmark, the seasonal
...
4. **Two classic findings are cheap and permanent.** Standalone seasonality is negative *before*
   costs with turnover well below the benchmark's, so the failure is the signal, not churn. Carry, as
...
families survive after costs, and whether a tuned ML variant earns its complexity?
...
0.3651, and both variants still clear it, so the verdict is not an artefact of the variants sitting
...
inside develop+validate, so no fold's fit or search ever saw an out-of-sample date.
...
0.199 per day, so the cost charge consumes essentially the whole signal. 6b holds 0.390 net, with a DSR
...
- **Max drawdown -0.691** (carry, develop+validate) means: the deepest peak-to-trough fall of the
  compounded equity curve. It is the number that survives a bad Sharpe, and the one that ends
