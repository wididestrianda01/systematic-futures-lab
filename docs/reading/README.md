...
## SHOULD schedule (read while building; each pinned to the phase that uses it)

| ID | Paper | Phase | Why there |
|---|---|---|---|
| S8 | Novy-Marx & Velikov 2016 — Taxonomy of Anomalies and Their Trading Costs | the trend sweep (engine) | legitimizes the linear cost model before wiring 0/5/10 bps sensitivity |
| S1 | Harvey, Liu, Zhu 2018 — Impact of Volatility Targeting | the families read (baseline) | target-level choice for the 10% vol overlay |
| S2 | Moreira & Muir 2017 — Volatility-Managed Portfolios | the families read (baseline) | counterparty evidence; prevents parroting one side in the memo |
| S6 | Asness, Frazzini, Pedersen 2012 — Leverage Aversion and Risk Parity | the families read (baseline) | portfolio-construction frame for the baseline |
