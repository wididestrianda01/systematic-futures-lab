...
$[-1,1]$ per symbol per day, a 10\% annualized volatility target overlay with $\pm1$ notional caps
applied identically, a linear cost charge of $b$ basis points per side on traded notional, and
metrics reported at $b \in \{0,2,5,10\}$ with $b=2$ the headline level a decision is read at. A
signal formed with data through day $t$ drives the position held from $t$ to $t+1$; a regression test
...
6a \texttt{ml\_defaults} & 1.406 & 0.799 & $-0.123$ & $-1.660$ \\
6b \texttt{ml\_tuned}    & 2.074 & 1.550 & 0.753 & $-0.621$ \\
\bottomrule
\end{tabular}
...
\section{Findings}
\begin{enumerate}
\item \textbf{Costs select strategies.} At 5\bps only XS momentum, the baseline and out-of-sample
seasonality remain positive; the trend benchmark, the seasonal tilt and both ML variants flip sign
between gross and headline.
\item \textbf{A null model is a real competitor.} Vol-targeted buy-and-hold earns 0.408 at 2\bps on
turnover of 0.112 --- the diversification return, which any signal must beat before claiming edge.
...
0.759 gross, 0.072 net, turnover 3.18 per day.
...
measured.
\item \textbf{Standalone seasonality fails on signal, not cost} ($-0.027$ gross at turnover 0.400,
below the benchmark's 1.654), on a score backed by five priors per calendar month.
\item \textbf{Carry's result is about the implementation under test.} The published construction is a
slope magnitude with rank weighting, monthly rebalancing and a 12-month-mean variant; what ran here is
the sign of the current basis, rebalanced daily (turnover 2.820, the highest of the classic set). The
sign convention is \emph{not} resolved by adopting the profitable direction; the carry premium is
neither confirmed nor refuted by these numbers.
...
the M7 book chapters are not read by the owner: the interpretation resting on them is marked as
...
\texttt{uv run python scripts/build\_phase3\_tables.py}, \texttt{build\_phase4\_tables.py},
\texttt{build\_results.1\_oot.py}; the committed tables are reproduced exactly. Reading order for the
