...
6a \texttt{ml\_defaults} & LightGBM defaults, purged/embargoed walk-forward, no search & M5, M6 \\
6b \texttt{ml\_tuned} & as 6a with a pruned TPE search per fold & M5, M6 \\
\bottomrule
\end{tabular}
\end{table}
...
book turnover of 0.007, which is the diversification return any signal must beat before claiming edge.
...
the trial-count DSR deflation plus the single out-of-sample read. The ML result is specific to this
feature set, label horizon, fold geometry, cost level and window; it is not a statement about
machine learning on futures in general.
